...
class Utils {
    ...
    static showCopyFeedback(input) {
        const btn = input.nextElementSibling; // Assuming button is next sibling
...
class DraftSimulator {
    constructor(config = null) {
        ...
        if (!config) {
            ...
            // 1. If we are in Game 2+, we MUST try current_draft_config first to preserve series state
            if (gameParam > 1) {
                const saved = localStorage.getItem('current_draft_config');
                if (saved) {
                    config = JSON.parse(saved);
                    console.log("Loaded current_draft_config for Game " + gameParam, config);
                    loaded = true;
                }
            }
    ...
    initUI() {
        ...
        if (nextCloseBtn) {
            nextCloseBtn.addEventListener('click', () => {
                ...
                window.location.href = `${baseUrl}?side=${side}&game=${nextGameIdx}`;
            });
        }
    }
    ...
    showDraftSummary() {
        ...
        // Match Result Button Listener REMOVED as per user request
        const resultBtn = document.getElementById('open-result-modal-btn');
        if (resultBtn) {
            resultBtn.style.display = 'none'; // Hide it
        }
        ...
        if (!nextBtn) {
            ...
            document.querySelector('#draft-summary-overlay .summary-footer').insertBefore(nextBtn, document.getElementById('summary-close-btn'));
        }

        const bestOf = parseInt(this.config.bestOf) || 1;
        if (this.activeGameIndex < bestOf) {
            nextBtn.style.display = 'inline-block';
            nextBtn.onclick = () => {
                overlay.classList.add('hidden');
                this.showNextGameLinks(this.activeGameIndex + 1);
            };
        } else {
    ...
    showNextGameLinks(index) {
        ...
        if (blueInput) {
            ...
            document.getElementById('next-link-red-input').value = `${baseUrl}?side=red&game=${index}`;
            document.getElementById('next-link-spec-input').value = `${baseUrl}?side=spectate&game=${index}`;
